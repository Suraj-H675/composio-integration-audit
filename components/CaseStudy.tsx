  ...
  return <main className="case-study">
    ...
    <section className="hero section-shell" id="top">
      ...
      <div className="hero-aside">
        <div className="aside-kicker">AUDIT FIELDS</div>
    ...
    <section className="section-shell insight-section" id="insight">
      <div className="section-intro split-intro"><div><SectionLabel index="02">Access model</SectionLabel><h2>API access has<br /><i>two gates.</i></h2></div><p>Customer access covers an account's own credentials. Public distribution covers a multi-customer integration. They are separate classifications.</p></div>
      ...
      <div className="insight-callout"><span className="callout-mark">!</span><p><strong>{metrics.distribution.customerEasyDistributionGated.numerator} apps allow self-service customer access but gate public distribution.</strong> These belong in a partnership or review queue.</p></div>
      <div className="breakdown-block"><div className="breakdown-head"><div><span className="eyebrow">DISTRIBUTION ACCESS / {data.dataset.appCount} APPS</span><h3>Distribution access</h3></div><span className="small-note">Final classifications · v2 rubric</span></div><StackedBar values={distributionValues} total={data.dataset.appCount} /><div className="legend-grid">{Object.entries(distributionValues).filter(([, value]) => value > 0).map(([key, value]) => <div className="legend-item" key={key}><span className={`legend-dot dot-${key}`} /><strong>{value}</strong><span>{valueLabel(key)}</span></div>)}</div></div>
    </section>

    <section className="section-shell mcp-section" id="mcp">
      <div className="section-intro split-intro"><div><SectionLabel index="03">MCP capability</SectionLabel><h2>Official MCP<br /><i>capability varies.</i></h2></div><p>{metrics.mcp.officialCount} vendors operate an official MCP. The type separates product action, documentation, and mixed servers.</p></div>
      <div className="mcp-layout"><div className="mcp-hero-stat"><div className="overline">OFFICIAL VENDOR MCP</div><strong>{metrics.mcp.officialCount}</strong><p>out of {data.dataset.appCount} audited apps</p><div className="mcp-stage-note"><span>{metrics.mcp.unknownStageAmongOfficial}</span> have no documented lifecycle label</div></div><div className="mcp-type-panel"><div className="panel-head"><span>Capability type among official MCPs</span><span>n = {metrics.mcp.officialCount}</span></div><BarList values={mcpTypeValues} total={metrics.mcp.officialCount} colors={["teal", "violet", "orange", "slate"]} /><div className="mcp-definition"><span><strong>Product-action</strong> operates on customer/product data. <strong>Documentation</strong> exposes vendor knowledge.</span></div></div></div>
      <div className="mcp-bottom"><div className="mcp-gap"><span className="eyebrow">NATIVE COVERAGE GAP</span><strong>{metrics.headline.productActionMcpAbsent.display}</strong><span>product-action MCP apps without a current Composio toolkit</span></div><div className="mcp-sanity"><span className="eyebrow">METRIC SCOPE</span><p>{metrics.mcp.sanityExplanation}</p></div></div>
    </section>

    <section className="section-shell action-section" id="opportunities">
      <div className="section-intro split-intro"><div><SectionLabel index="04">Action</SectionLabel><h2>Three action<br /><i>lanes.</i></h2></div><p>Build open opportunities. Pursue gated distribution. Support customer-managed connections.</p></div>
    ...
    <section className="section-shell category-section" id="categories">
      <div className="section-intro split-intro"><div><SectionLabel index="05">Categories</SectionLabel><h2>Patterns by<br /><i>category.</i></h2></div><p>Buildability is highest in developer tools and productivity. Commerce and AI/media have more access and distribution uncertainty.</p></div>
    ...
    <section className="section-shell method-section" id="method">
      <div className="section-intro split-intro"><div><SectionLabel index="06">Method</SectionLabel><h2>Evidence<br /><i>pipeline.</i></h2></div><p>Each claim carries a source, retrieval method, timestamp, and confidence. Important claims receive independent challenge and human review.</p></div>
    ...
    <section className="section-shell unknown-section">
      <div className="unknown-card"><div className="unknown-index">?</div><div className="unknown-copy"><SectionLabel index="07">Identity ambiguity</SectionLabel><h2>Paygent Connect<br /><i>remains unresolved.</i></h2><p>“Paygent Connect — paygent (NMI-powered)” did not reconcile cleanly with the current first-party evidence. The pipeline kept the identity unresolved instead of attaching API claims to a similarly named product.</p><div className="unknown-tags"><span>Assignment hint: <strong>{paygent?.assignmentHint}</strong></span><StatusPill value="unresolved" /><span>{metrics.headline.unresolvedIdentities.display} unresolved identities in total</span></div><div className="unknown-evidence">{paygent?.identityEvidence.slice(0, 2).map((item) => <EvidenceLink evidence={item} key={item.url} />)}</div></div><div className="unknown-side"><span className="eyebrow">UNRESOLVED IDENTITIES</span><strong>{metrics.headline.unresolvedIdentities.display}</strong><p>remain unresolved in the final dataset.</p></div></div>
    </section>

    <section className="section-shell verification-section" id="verification">
      <div className="section-intro split-intro"><div><SectionLabel index="08">Holdout</SectionLabel><h2>Human<br /><i>verification.</i></h2></div><p>Resolved-field accuracy comes from a preregistered {metrics.holdout.appCount}-app human-reviewed holdout. It is not an accuracy claim about the entire {data.dataset.appCount}-app dataset.</p></div>
    ...
    <section className="section-shell explorer-section" id="explore">
      <div className="explorer-head"><div><SectionLabel index="09">Explore the dataset</SectionLabel><h2>Explore all<br /><i>{data.dataset.appCount} apps.</i></h2></div><div className="explorer-summary"><strong>{filteredApps.length}</strong><span>matching apps</span><small>Every row opens its evidence trail</small></div></div>
