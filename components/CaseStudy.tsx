  ...
  return <main className="case-study">
    ...
    <section className="hero section-shell" id="top">
      <div className="hero-copy">
        <h1>{data.dataset.appCount}-app integration audit.<br /><span>Buildability and distribution.</span></h1>
        <p className="hero-support">{data.dataset.appCount} apps across {Object.keys(metrics.categoryPatterns).length} categories. Each record covers identity, access, APIs, MCP, Composio coverage, and evidence.</p>
      </div>
      <div className="hero-aside">
        <div className="lens-row"><strong>Technical buildability</strong><small>Interface available</small></div>
        <div className="lens-row active"><strong>Distribution access</strong><small>Customer authorization path</small></div>
        <div className="lens-row"><strong>Evidence quality</strong><small>Source-backed fields</small></div>
      </div>
    </section>
    ...
    <section className="section-shell insight-section" id="insight">
      <div className="section-intro split-intro"><div><h2>API access has<br /><i>two gates.</i></h2></div><p>Customer access covers an account's own credentials. Public distribution covers a multi-customer integration. They are separate classifications.</p></div>
      <div className="gate-grid">
        <div className="gate-card customer-gate"><div className="gate-number">01</div><h3>Customer credentials</h3><p>Credentials or authorization for the customer's own account.</p><div className="gate-stat"><strong>{selfServeCount}</strong><span>apps with a self-serve customer path</span></div></div>
        <div className="gate-card distribution-gate"><div className="gate-number">02</div><h3>Public distribution</h3><p>OAuth, review, partner, or vendor-approval path for public distribution.</p><div className="gate-stat"><strong>{metrics.distribution.documentedDistributionGates.numerator}</strong><span>apps with a documented distribution gate</span></div></div>
      </div>
      <div className="insight-callout"><p><strong>{metrics.distribution.customerEasyDistributionGated.numerator} apps allow self-service customer access but gate public distribution.</strong> These belong in a partnership or review queue.</p></div>
      <div className="breakdown-block"><div className="breakdown-head"><div><h3>Distribution access</h3></div></div><StackedBar values={distributionValues} total={data.dataset.appCount} /><div className="legend-grid">{Object.entries(distributionValues).filter(([, value]) => value > 0).map(([key, value]) => <div className="legend-item" key={key}><span className={`legend-dot dot-${key}`} /><strong>{value}</strong><span>{valueLabel(key)}</span></div>)}</div></div>
    </section>

    <section className="section-shell mcp-section" id="mcp">
      <div className="section-intro split-intro"><div><h2>Official MCP<br /><i>capability varies.</i></h2></div><p>{metrics.mcp.officialCount} vendors operate an official MCP. The type separates product action, documentation, and mixed servers.</p></div>
      <div className="mcp-layout"><div className="mcp-hero-stat"><strong>{metrics.mcp.officialCount}</strong><p>official MCPs across {data.dataset.appCount} audited apps</p><div className="mcp-stage-note"><span>{metrics.mcp.unknownStageAmongOfficial}</span> have no documented lifecycle label</div></div><div className="mcp-type-panel"><div className="panel-head"><span>Capability type among official MCPs</span></div><BarList values={mcpTypeValues} total={metrics.mcp.officialCount} colors={["teal", "violet", "orange", "slate"]} /><div className="mcp-definition"><span><strong>Product-action</strong> operates on customer/product data. <strong>Documentation</strong> exposes vendor knowledge.</span></div></div></div>
      <div className="mcp-bottom"><div className="mcp-gap"><strong>{metrics.headline.productActionMcpAbsent.display}</strong><span>product-action MCP apps without a current Composio toolkit</span></div><div className="mcp-sanity"><span className="eyebrow">METRIC SCOPE</span><p>{metrics.mcp.sanityExplanation}</p></div></div>
    </section>

    <section className="section-shell action-section" id="opportunities">
      <div className="section-intro split-intro"><div><h2>Three action<br /><i>lanes.</i></h2></div><p>Build open opportunities. Pursue gated distribution. Support customer-managed connections.</p></div>
      <div className="lane-grid"><div className="lane lane-build"><div className="lane-head"><span className="lane-index">A</span><div><h3>Build now</h3><p>Open distribution + accessible credentials</p></div><span className="lane-count">{metrics.opportunities.easyWins.length}</span></div><div className="opportunity-list">{metrics.opportunities.easyWins.map((item) => <OpportunityCard key={item.app} opportunity={item} app={appByName.get(item.app)} kind="easy" />)}</div></div><div className="lane lane-partner"><div className="lane-head"><span className="lane-index">B</span><div><h3>Partner / get approved</h3><p>Technology available; distribution gated</p></div><span className="lane-count">{metrics.opportunities.partnership.length}</span></div><div className="opportunity-list">{metrics.opportunities.partnership.map((item) => <OpportunityCard key={item.app} opportunity={item} app={appByName.get(item.app)} kind="partnership" />)}</div></div><div className="lane lane-customer"><div className="lane-head"><span className="lane-index">C</span><div><h3>Customer-managed</h3><p>Customer credentials and setup</p></div><span className="lane-count">{metrics.opportunities.customerManaged.length}</span></div><div className="customer-managed-summary"><strong>{metrics.opportunities.customerManaged.length}</strong><p>technically useful apps where each customer manages their own credentials or setup.</p><details><summary>See all {metrics.opportunities.customerManaged.length}</summary><div className="name-cloud">{metrics.opportunities.customerManaged.map((item) => <button key={item.app} onClick={() => setSelectedApp(appByName.get(item.app) ?? null)}>{item.app}</button>)}</div></details></div></div></div>
    </section>

    <section className="section-shell category-section" id="categories">
      <div className="section-intro split-intro"><div><h2>Patterns by<br /><i>category.</i></h2></div><p>Buildability is highest in developer tools and productivity. Commerce and AI/media have more access and distribution uncertainty.</p></div>
